...
const envSchema = z.object({
  ...
  OLLAMA_LLM_MODEL: z.string().default('gemma3:8b'),
  OLLAMA_EMBEDDING_DIMENSIONS: z.string().default('768').transform(Number),
  PATHSEEKR_DATA_DIR: z.string().default('~/.pathseekr'),
  INDEXING_CONCURRENCY: z.string().default('10').transform(Number),
  EMBEDDING_BATCH_SIZE: z.string().default('20').transform(Number),
  MAX_FILE_SIZE_BYTES: z.string().default('1048576').transform(Number),
  DEFAULT_SEARCH_LIMIT: z.string().default('10').transform(Number),
  BM25_WEIGHT: z.string().default('0.5').transform(Number),
  API_PORT: z.string().default('3001').transform(Number),
  MCP_PORT: z.string().default('3002').transform(Number),
  WEB_PORT: z.string().default('3000').transform(Number),
...
function buildConfig(): PathseekrConfig {
  ...
  if (!parsed.success) {
    process.stderr.write('Pathseekr MCP: invalid configuration\n')
    process.exit(1)
  }
